export class RegisterView {
    ...
    return `
      <div class="auth-container">
        <div class="auth-card card">
          ...
          <h2 style="text-align: center; margin-bottom: 2rem; font-size: 1.5rem;">Crear Cuenta</h2>
          <form id="registerForm">
            <div class="form-group">
              <label for="name" class="form-label">Nombre</label>
              <input type="text" id="name" class="form-input" autocomplete="name">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem;">
                <small style="color: var(--color-gray-400); font-size: 0.75rem;">Mínimo 8 caracteres</small>
          ...
          <p style="text-align: center; margin-top: 1.5rem; color: var(--color-gray-400);">
            ¿Ya tienes cuenta? <a href="/login" data-link style="color: var(--color-primary);">Inicia sesión</a>
          </p>
        </div>
      </div>
    `;
  ...
  async mounted() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const { user, token } = await window.api.auth.register({ name, email, password });
        localStorage.setItem('e3_token', token);
        window.store.set('user', user);
        window.store.set('isAuthenticated', true);

        window.toast.success('Cuenta creada correctamente');
        window.location.href = '/dashboard';
      } catch (error) {
        window.toast.error(error.message || 'Error al crear cuenta');
      }
    });
  }
}
